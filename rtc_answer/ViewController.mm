...
- (void)viewDidLoad {
    ...
    answer_ = new answerer();
    answer_->setSdpCallback([=](std::string sdp) {
    ...
    answer_->setCandidateCallback([=](std::string candidate) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)sendClicked:(id)sender {
    answer_->printConnectionInfo();
    std::string msg = [localMessageView.string UTF8String];
    answer_->setMessage(msg);
}
...
- (void)sdpClicked:(id)sender {
    ...
    NSLog(@"%@", remoteSdpView.string);
    std::cout  << remoteSdpView.string.UTF8String << std::endl;
    answer_->setRemoteSdp([remoteSdpView.string UTF8String]);
...
- (void)candicateClicked:(id)sender {
    ...
    NSLog(@"%@", remoteCandidateView.string);
    answer_->setRemoteCandidate([remoteCandidateView.string UTF8String]);
}
@end
